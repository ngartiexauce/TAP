...
// G, entre la position G.start et G.end, et renvoyer son coût. Bien
// sûr le chemin doit être calculé selon l'algorithme A* vu en cours
// (utilisez les notes de cours !), algorithme paramétré par
// l'heuristique h() (à choisir lors de l'appel parmi celles ci-dessus
...
// d'affichage et de progression de l'algorithme par
...
// de Dijkstra). Pensez qu'avec la gestion paresseuse du tas, la
// taille de Q est au plus la somme des degrés des sommets dans la
// grille. Pour visualiser un noeud de coordonnées (i,j) qui passe
// dans le tas Q vous pourrez mettre G.mark[i][j] = M_FRONT au moment
// où vous l'ajoutez. Même si cela est tentant, il ne faut pas
// utiliser la marque M_FRONT pour savoir si un sommet est dans Q. Le
// champs .mark ne doit servir que pour l'ensemble P et l'affichage de
// la grille. Pas pour le tas Q !
bool are_equals(position A, position B){
  if(A.x == B.x && A.y == B.y){
    return true;
  }
  return false;
...
void supprimer_rec(node s){
  if(s!= NULL){
    node p = s->parent;
    free(s);
    if(p != NULL && p->nchild -- == 0){
      supprimer_rec(p);
    }
  }
}
void memory_wash(heap h){
 /* while(h->n !=0){
    node s = (node)heap_get(h,h->n);
    supprimer_rec(s);
    h->n--;
  } */
  while(!heap_empty(h)){
    node s= (node)heap_pop(h);
    printf("%f\n",s->score );
    supprimer_rec(s);
  }
}
...
double A_star(grid G, heuristic h){
  if(G.mark[G.end.x][G.end.y] == V_WALL || G.mark[G.start.x][G.start.y] == V_WALL ){
    return -1;
  }
  int size = 4;
  heap Q = heap_create(size,f);
  int x = G.start.x;
  int y = G.start.y;
  double cost = 0;
  double score = h(G.start,G.end, &G);
  node s = createNode(x, y, cost, score,  NULL);
  heap_add(Q,s);
  ...
  while(!heap_empty(Q)){
    ...
    if(are_equals(u->pos,G.end)){

      double d = u->cost;

      for(uint i=0; i<G.X; i++){
        for(uint j=0; j < G.Y; j++){
      ...
      memory_wash(Q);
      printf("La taille de Q avant destroy: %lu\n", sizeof(Q));
      heap_destroy(Q);
      return d;
    }
    ...
    int voisin_trouve=0;
    for(uint i=u->pos.x-1; i <= u->pos.x+1; i++){
      for(uint j=u->pos.y-1; j <= u->pos.y+1; j++){
        if(G.value[i][j] != V_WALL && !( i==u->pos.x && j==u->pos.y) && G.mark[i][j] != M_USED){
          double c = u->cost + weight[G.value[i][j]];
          position p;
          p.x=i;
          p.y=j;
          double s_ajust; // une constante qui permet de dimunier le score pour favoriser les chemins horizontaux et verticaux.
          s_ajust = 0;
          if(i == u->pos.x || j== u->pos.y){
            s_ajust = 0.001;
          }
          double score = c + h(p,G.end,&G) - s_ajust;
          node v = createNode(i,j,c,score,u);
          u->nchild++;
          heap_add(Q,v);
          voisin_trouve++;
  ...
  // Vous pouvez réfléchir à une solution simple pour libérer tous les
...
//     bien des arêtes horizontales que diagonales (qui ont le même
//     coût), même pour des chemins en ligne droite. Essayer de
...
//     la distance entre .start et .end.
//
...
//     place dans le tableau, on double sa taille avec un
//     realloc(). On peut imaginer que l'ancien paramètre 'nmax'
//     devienne non pas le nombre maximal d'éléments, mais sa taille
//     maximale initiale (comme par exemple nmax=4).
//
...
//     node de P ou Q possède. C'est relativement léger à gérer
//     puisqu'on augmente .nchild de u chaque fois qu'on fait
//     parent[v]=p, soit juste après "node v = createNode(p,...)".
//     Pensez à faire .nchild=0 dans createNode(). Notez bien qu'ici
//     on parle de "node", donc des copies de sommets.
//
...
//     qui sont des feuilles. L'observation est alors que de tels
//     nodes ne peuvent pas être sur le chemin s->t. On peut donc les
//     supprimer au fur et à mesure directement dans la boucle
//     principale quand on les détecte. On voit qu'un tel node
//     apparaît si après avoir parcouru tous les voisins de u aucun
//     node v n'est créé (et ajouté dans Q). Il suffit donc de savoir
//     si on est passé par heap_add() (ou encore de comparer la taille
//     de Q avant et après la boucle sur les voisins). Si u est une
//     feuille, on peut alors supprimer le node u, mettre à jour
//     .nchild de son père et remonter la branche jusqu'à trouvé un
//     node qui n'est plus une feuille. C'est donc la même procédure
//     d'élagage que précdemment qu'on pourrait capturer par une
//     fonction freeNode(node p).
...
int main(int argc, char *argv[]){
  ...
  G.start=(position){0.2*G.X,0.5*G.Y}, G.end=(position){0.8*G.X,0.9*G.Y};
  ...
  drawGrid(G); // dessin de la grille avanERRORt l'algo
